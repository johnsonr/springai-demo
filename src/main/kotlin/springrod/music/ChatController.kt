...
internal class ChatController(
    ...
    @PostMapping("/logout")
    fun destroySession(request: HttpServletRequest): ResponseEntity<String> {
        // Invalidate HTTP session if it exists
        request.getSession(false)?.invalidate()
        return ResponseEntity.ok()
            .header("HX-Refresh", "true") // This tells HTMX to refresh the page
            .body("Session destroyed successfully")
    }
